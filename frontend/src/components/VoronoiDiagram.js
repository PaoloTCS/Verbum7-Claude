...
const VoronoiDiagram = ({ 
  ...
  useEffect(() => {
    ...
    // All functions moved inside the effect to avoid dependency issues
    
    // Function to position domains
    const positionDomainsInEffect = (domainsArray, distances, w, h) => {
      // Copy domains to avoid mutating props
      const positionedDomains = [...domainsArray];
      ...
      const positionWithForces = (doms, sDistances, wid, hgt) => {
        // Create a copy of domains that we can mutate
        const positioned = doms.map(domain => ({
          ...domain,
          x: domain.x || Math.random() * wid,
          y: domain.y || Math.random() * hgt
        }));
        
        // Convert semantic distances to links for force layout
        const links = [];
        Object.entries(sDistances).forEach(([pair, distance]) => {
          const [id1, id2] = pair.split('|');
          const source = positioned.findIndex(d => d.id === id1);
          const target = positioned.findIndex(d => d.id === id2);
        ...
        const iterationCount = doms.length > 20 ? 100 : 300;
        
        // Create force simulation
        const simulation = d3.forceSimulation(positioned)
          .force('charge', d3.forceManyBody().strength(-300))
          .force('center', d3.forceCenter(wid / 2, hgt / 2))
        ...
        // Run simulation with limited iterations for larger datasets
        for (let i = 0; i < iterationCount; ++i) simulation.tick();
        
        // Make sure domains stay within bounds
        positioned.forEach(domain => {
          domain.x = Math.max(50, Math.min(wid - 50, domain.x));
          domain.y = Math.max(50, Math.min(hgt - 50, domain.y));
        });
        
        return positioned;
      };
      
      const arrangeInCircle = (doms, wid, hgt) => {
        const centerX = wid / 2;
        const centerY = hgt / 2;
        const radius = Math.min(wid, hgt) / 2.5;
        
        return doms.map((domain, i) => ({
          ...domain,
          x: centerX + radius * Math.cos((i / doms.length) * 2 * Math.PI),
          y: centerY + radius * Math.sin((i / doms.length) * 2 * Math.PI)
        }));
      };
      
      // If we have semantic distances, use force layout
      if (Object.keys(distances).length > 0) {
        return positionWithForces(positionedDomains, distances, w, h);
      }
      
      // Otherwise, arrange in a circle
      return arrangeInCircle(positionedDomains, w, h);
    };
    
    // Function to create the Voronoi diagram
    const createVoronoiDiagramInEffect = (positionedDomains) => {
      ...
      const delaunay = d3.Delaunay.from(positionedDomains, d => d.x, d => d.y);
      const voronoi = delaunay.voronoi([0, 0, width, height]);
      
      // Draw cells
      const cells = svg.selectAll('g.cell')
        .data(positionedDomains)
        .enter()
        .append('g')
        .attr('class', 'cell')
      ...
      cells.append('path')
        ...
        .attr('fill', (d, i) => d3.interpolateRainbow(i / positionedDomains.length))
      ...
      positionedDomains.forEach(domain => {
        if (domain.documents && domain.documents.length > 0) {
          ...
          docGroup.selectAll('circle.doc-icon')
            ...
            .attr('class', 'doc-icon')
            .attr('cx', (d, i) => {
              // Improve spacing for multiple documents
    ...
    const positionedDomains = positionDomainsInEffect(domains, semanticDistances, width, height);
    
    // Create D3 visualization
    createVoronoiDiagramInEffect(positionedDomains);
